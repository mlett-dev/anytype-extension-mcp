...
const iconPayloadDescription = "The object's icon. Three shapes are accepted: an emoji, {\"format\":\"emoji\",\"emoji\":\"🍰\"}; an uploaded image, {\"format\":\"file\",\"file\":\"<image object id from file-upload>\"}, which is how a custom picture becomes an icon; or a built-in symbol, {\"format\":\"icon\",\"name\":\"book\",\"color\":\"red\"}. The icon is a small square next to the title — for the wide banner above it use object-set-cover instead."
...
type mcpServer struct {
	...
	// lastRawParams is the tools/call params exactly as they arrived, kept for
	// debug-receive-file: the map[string]any the dispatchers get has already
	// lost _meta and the original JSON types, and for a tool whose whole
	// purpose is to report what the connector runtime sent, that lost detail is
	// the answer. The run loop handles one message at a time, so there is
	// nothing here to race with.
	lastRawParams json.RawMessage
}

func main() {
	cfg := serverConfig{
		grpcAddr:      getEnv("ANYTYPE_GRPC_ADDR", anytypefiles.DefaultGRPCAddress),
		token:         strings.TrimSpace(os.Getenv("ANYTYPE_SESSION_TOKEN")),
		timeout:       parseDurationEnv("ANYTYPE_TIMEOUT", anytypefiles.DefaultTimeout),
		...
		leanToolset:   strings.EqualFold(getEnv("ANYTYPE_MCP_TOOLSET", "full"), "lean"),
		unsplashKey:   strings.TrimSpace(os.Getenv("UNSPLASH_ACCESS_KEY")),
	}
...
func (s *mcpServer) handleRequest(req rpcRequest) (rpcResponse, bool) {
	...
	case "tools/list":
		return rpcResponse{
			JSONRPC: "2.0",
			ID:      req.ID,
			Result:  map[string]any{"tools": s.allToolDefs()},
		}, req.ID != nil
	case "tools/call":
		if req.ID == nil {
			return rpcResponse{}, false
...
func (s *mcpServer) callTool(name string, args map[string]any) (map[string]any, error) {
	...
	for _, dispatch := range s.extraDispatchers() {
		if res, err, handled := dispatch(name, args); handled {
			return res, err
		}
	}
...
// allToolDefs is every tool this server offers. It backs tools/list and the
// count that server-info reports, so the two cannot disagree.
func (s *mcpServer) allToolDefs() []any {
	...
	return append([]any{
		...
		map[string]any{
			"name":        "file-upload",
			"description": fmt.Sprintf("Upload a staged local file from the configured input root to Anytype. input_root=%q, server_input_root=%q. staged_path may be absolute under input_root or a relative_path previously returned by file-list-input. IMPORTANT: use the exact filename/path as listed on disk. Spaces are allowed and must not be rewritten to underscores. The returned object_id is a normal Anytype file or image object: pass it to object-set-cover for a cover, or to update-object-compact as {\"format\":\"file\",\"file\":\"<object_id>\"} for an icon. This tool can only reach files that are already in the input root of THIS server; for a file your own runtime holds, use file-upload-attachment instead.", s.cfg.inRoot, s.cfg.serverInRoot),
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
				...
				"items": map[string]any{
					...
					"items": map[string]any{
						...
						"properties": map[string]any{
							...
							"icon": map[string]any{
								"type":                 "object",
								"description":          iconPayloadDescription,
								"additionalProperties": true,
							},
							"properties": map[string]any{
								"type":        "array",
		...
		map[string]any{
			...
			"description": "List the object types of a space. Use this to discover type keys for create-objects-compact-many and type IDs for the type tools. Property definitions and icons are omitted here because they multiply by the number of types: pass include_properties=true to include them, or read a single type with get-type-compact, which returns them by default. Do not confuse type fields with object fields.",
		...
		map[string]any{
			...
			"description": "Read one object type, including the properties linked to it — a type's properties are returned by default here, because they are what defines the type. Use this before property updates to inspect property keys and formats. type_id is the bafyrei... id from list-types-compact; the type key is NOT accepted and returns 500. Pass include_properties=false to omit the properties.",
			// include_properties defaults to true here and to false in
			// list-types-compact, and the two share the option helper — so the
			// entry has to be overridden after it, or the schema advertises
			// list-types-compact's default for a tool that does not use it.
			"inputSchema": withOverrides(compactTypeToolSchema([]string{"space_id", "type_id"}, map[string]any{
				"space_id": map[string]any{
					"type":        "string",
					"description": "Anytype space ID.",
				},
				"type_id": map[string]any{
					"type":        "string",
					"description": "Type ID: the bafyrei... id from list-types-compact. The type key is NOT accepted here and returns 500.",
				},
			}), map[string]any{
				"include_properties": map[string]any{
					"type":        "boolean",
					"description": "Include compact linked property definitions. Defaults to true for this tool, because a type's properties are what defines it. If property_keys is empty, only max_properties are included.",
					"default":     true,
				},
			}),
		},
		map[string]any{
			...
			"description": "REST wrapper for Anytype get-list-views with compact view metadata for a collection/set. Use this to find the view_id required by get-list-objects-compact. Omits filters and sorts unless include_filters/include_sorts are true.\n\n" +
				"The filters it returns are LOSSY, and not because of this wrapper: the REST endpoint types a filter value as a string, so anything that is not one comes back as \"\". A checkbox filter reads as value \"\" instead of false, and an object filter as \"\" instead of its list of ids. Conditions and layouts also arrive in the REST spelling (eq, ne, nempty, grid) rather than the one the query-* tools take. Use query-inspect when the actual filter values matter \u2014 it reads the same views over gRPC and reports them typed and in the write vocabulary.",
...
// extraToolDefs collects the tool groups that live outside the literal above.
// Their order is the order a client sees them in tools/list.
func (s *mcpServer) extraToolDefs() []any {
	var defs []any
	for _, group := range [][]any{
		s.restToolDefs(), s.queryToolDefs(), s.blockToolDefs(),
		s.tableToolDefs(), s.objectToolDefs(), s.transferToolDefs(),
		s.extrasToolDefs(), s.spaceOpsToolDefs(), s.coverToolDefs(),
		s.usageToolDefs(), s.serverInfoToolDefs(),
		s.debugToolDefs(), s.openAIFileToolDefs(), s.attachmentToolDefs(),
	} {
...
// toolDispatcher handles a tool if it owns the name, reporting that via the
// bool so the next group gets a turn otherwise.
type toolDispatcher func(name string, args map[string]any) (map[string]any, error, bool)

func (s *mcpServer) extraDispatchers() []toolDispatcher {
	return []toolDispatcher{
		s.dispatchRestTool, s.dispatchQueryTool, s.dispatchBlockTool,
		s.dispatchTableTool, s.dispatchObjectTool, s.dispatchTransferTool,
		s.dispatchExtrasTool, s.dispatchSpaceOpsTool, s.dispatchCoverTool,
		s.dispatchUsageTool, s.dispatchServerInfoTool,
		s.dispatchDebugTool, s.dispatchAttachmentTool,
	}
}
